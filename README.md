...
# Aquí registras lo que ven los tarotistas y lo que marcan las varillas
...
def calcular_marcador_exacto(partido_id):
    ...
    # Procesar datos de Radiestesia (Varillas)
    ...
    # Consolidador Místico (Promedios ponderados)
    cant_tarot = len(datos["tarotistas"])
    cant_varillas = len(datos["radiestesia"])
    total_fuentes = cant_tarot + cant_varillas

    # Cálculo final del marcador exacto aplicando redondeo matemático
    marcador_final_e1 = math.round((total_goles_e1_tarot + total_goles_e1_varillas) / total_fuentes)
    marcador_final_e2 = math.round((total_goles_e2_tarot + total_goles_e2_varillas) / total_fuentes)
    
    # Índice de coincidencia espiritual
    alineacion_cosmica = (fuerza_tarot_total + (energia_e1_varillas - energia_e2_varillas + 50)) / 2

    # 3. PRESENTACIÓN DE RESULTADOS (Consola)
    print("==================================================")
    print(f"🔮 SISTEMA MURIOS MATÍAS - SIMULADOR MÍSTICO 🔮")
...
# Ejecutar el simulador para el próximo partido
